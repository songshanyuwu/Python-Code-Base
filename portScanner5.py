...
def portScanner(host,port):
    ...
    try:
        ...
        lock.acquire()  # 获取锁
        openNum+=1
        print('[{:^8}] {:>15} {:>6} open'.format(time_str,host,port))
        lock.release()  # 锁释放
        s.close()
    except:
        pass
...
def get_ac_ip(ip_list):
    try:
        ...
        print('The current user permissions unable to send icmp packets /n Please run with administrator privileges')
        return ip_list
...
-p port: 1-65535 | 22,80,443,3389 | port.ini | Null
    ...
    try:
        ...
        host_list = get_ac_ip(host_list)
        port_list = get_port_list(portstr)
        # 开始扫描端口
        ThreadNum(set(host_list),port_list)
...
# -h 172.16.21.60-70 -p 22,80,443,3306,3389
